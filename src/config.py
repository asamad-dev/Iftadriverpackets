...
class Config:
    ...
    # Rate Limiting
    NOMINATIM_RATE_LIMIT: float = float(os.getenv('NOMINATIM_RATE_LIMIT', '1.0'))  # seconds between requests
    HERE_RATE_LIMIT: float = float(os.getenv('HERE_RATE_LIMIT', '0.1'))  # seconds between requests
    ...
    LOG_FILE_MAX_BYTES: int = int(os.getenv('LOG_FILE_MAX_BYTES', str(5 * 1024 * 1024)))  # 5MB
    LOG_BACKUP_COUNT: int = int(os.getenv('LOG_BACKUP_COUNT', '7'))  # Keep 7 days of daily logs
    
    # Session Logging Settings
    LOG_SESSION_ENABLED: bool = os.getenv('LOG_SESSION_ENABLED', 'true').lower() == 'true'
    LOG_DAILY_ROTATION: bool = os.getenv('LOG_DAILY_ROTATION', 'false').lower() == 'true'  # Disabled by default
    LOG_SIZE_ROTATION: bool = os.getenv('LOG_SIZE_ROTATION', 'false').lower() == 'true'   # Disabled by default
    
    # Log Cleanup Settings  
    LOG_SESSION_CLEANUP_DAYS: int = int(os.getenv('LOG_SESSION_CLEANUP_DAYS', '30'))  # Keep session logs for 30 days
    ...
    USE_HERE_API_PREFERRED: bool = os.getenv('USE_HERE_API_PREFERRED', 'true').lower() == 'true'
    
    # Route Analysis Configuration
    MIN_STATE_MILES_THRESHOLD: float = float(os.getenv('MIN_STATE_MILES_THRESHOLD', '1.0'))
    ...
    DEFAULT_REFERENCE_CSV: str = os.getenv('DEFAULT_REFERENCE_CSV', 'input/driver - Sheet1.csv')
    
    # State Boundary Data
    STATE_SHAPEFILE_PATH: str = os.getenv('STATE_SHAPEFILE_PATH', 'src/cb_2024_us_state_500k.shp')
    ...
    MAX_GALLONS_PER_PURCHASE: float = float(os.getenv('MAX_GALLONS_PER_PURCHASE', '500.0'))
    MIN_GALLONS_PER_PURCHASE: float = float(os.getenv('MIN_GALLONS_PER_PURCHASE', '0.1'))
    ...
    def validate_configuration(cls) -> Dict[str, Any]:
        ...
        if not cls.GEMINI_API_KEY:
            ...
            validation_result['errors'].append('GEMINI_API_KEY is required for data extraction')
            validation_result['is_valid'] = False
        
        if not cls.HERE_API_KEY:
            validation_result['warnings'].append('HERE_API_KEY not configured - geocoding will use Nominatim fallback')
        
        # Validate timeout values
        if cls.GEMINI_TIMEOUT < 10:
            validation_result['warnings'].append(f'GEMINI_TIMEOUT ({cls.GEMINI_TIMEOUT}s) is very low, may cause timeouts')
        
        if cls.GEOCODING_TIMEOUT < 1:
            validation_result['warnings'].append(f'GEOCODING_TIMEOUT ({cls.GEOCODING_TIMEOUT}s) is very low')
        
        # Validate retry configuration
        if cls.MAX_RETRIES > 10:
            validation_result['warnings'].append(f'MAX_RETRIES ({cls.MAX_RETRIES}) is very high')
        
        if cls.RETRY_DELAY < 0.1:
            validation_result['warnings'].append(f'RETRY_DELAY ({cls.RETRY_DELAY}s) is very low')
        
        # Validate thresholds
        if cls.MIN_TOTAL_MILES >= cls.MAX_TOTAL_MILES:
            validation_result['errors'].append('MIN_TOTAL_MILES must be less than MAX_TOTAL_MILES')
            validation_result['is_valid'] = False
        
        # Validate log level
        valid_log_levels = ['DEBUG', 'INFO', 'WARNING', 'ERROR', 'CRITICAL']
        if cls.LOG_LEVEL not in valid_log_levels:
            validation_result['warnings'].append(f'LOG_LEVEL "{cls.LOG_LEVEL}" not recognized, using INFO')
            cls.LOG_LEVEL = 'INFO'
        ...
        for path_name, path_value in []:  # No required paths currently
            if not Path(path_value).exists():
                validation_result['warnings'].append(f'{path_name} path does not exist: {path_value}')
    ...
    def print_configuration_summary(cls) -> None:
        ...
        # API Configuration
        print("\n📡 API Configuration:")
        print(f"  Gemini API: {'✅ Configured' if cls.GEMINI_API_KEY else '❌ Missing'}")
        print(f"  HERE API: {'✅ Configured' if cls.HERE_API_KEY else '⚠️ Not configured (will use fallback)'}")
