...
class DataValidator:
        ...
        if data.get("trailer"):
            ...
            if len(trailer) == 3 and trailer.isdigit():
                if not trailer.startswith("2"):
                    ...
                    corrections.append(
                        f"Trailer number corrected: {original_trailer} → {corrected_trailer}"
                    )
        ...
        for field in location_fields:
            if data.get(field):
                ...
                if corrected_value != original_value:
                    ...
                    corrections.append(
                        f"{field} corrected: {original_value} → {corrected_value}"
                    )
        ...
        for field in date_fields:
            if data.get(field):
                ...
                if corrected_date != original_date:
                    ...
                    corrections.append(
                        f"{field} format corrected: {original_date} → {corrected_date}"
                    )
    ...
    def _handle_dropoff_arrays(self, data: Dict) -> List[str]:
        ...
        if data.get("drop_off"):
            ...
            if isinstance(drop_off_value, str) and " to " in drop_off_value.lower():
                ...
                drop_off_array = [
                    loc.strip() for loc in drop_off_value.split(" to ") if loc.strip()
                ]
                ...
                corrections.append(
                    f"drop_off converted to array: {drop_off_value} → {corrected_drop_offs}"
                )
    ...
    def _correct_field_duplication(self, data: Dict) -> List[str]:
        ...
        for field in ["second_drop", "third_drop", "forth_drop"]:
            if data.get(field) and data[field] in [inbound_pu, drop_off_str]:
                ...
                data[field] = ""
                corrections.append(
                    f"{field} cleared (matched inbound_pu/drop_off): {original_value} → empty"
                )
    ...
    def _clear_duplicate_middle_drops(self, data: Dict) -> List[str]:
        ...
        end_loc = (
            ", ".join(end_loc_value)
            if isinstance(end_loc_value, list)
            else str(end_loc_value)
        )
        ...
        if start_loc and end_loc and start_loc == end_loc:
            for middle_field in ["second_drop", "third_drop", "forth_drop"]:
                if data.get(middle_field) and data[middle_field].strip() == start_loc:
                    ...
                    data[middle_field] = ""
                    corrections.append(
                        f"{middle_field} cleared (same as start/end location {start_loc})"
                    )
    ...
    def _aggregate_fuel_by_state(self, fuel_purchases: List[Dict]) -> Dict:
        ...
        processed_data = {"aggregated_by_state": {}, "total_gallons": 0, "warnings": []}
        ...
        for purchase in fuel_purchases:
            if not isinstance(purchase, dict):
                processed_data["warnings"].append(
                    f"Invalid fuel purchase data format: {purchase}"
                )
                continue
            ...
            if not state or state not in valid_states:
                processed_data["warnings"].append(
                    f"Invalid or missing state: '{purchase.get('state', '')}' - skipping fuel entry"
                )
                continue
            ...
            try:
                ...
                clean_gallons = "".join(
                    c for c in gallons_str if c.isdigit() or c == "."
                )
                gallons = float(clean_gallons) if clean_gallons else 0.0

                if gallons <= 0:
                    processed_data["warnings"].append(
                        f"Invalid gallons amount: '{gallons_str}' for state {state} - skipping"
                    )
                    continue

            except (ValueError, TypeError):
                processed_data["warnings"].append(
                    f"Could not parse gallons: '{gallons_str}' for state {state} - skipping"
                )
                continue
        ...
        if data.get("trip"):
            try:
                ...
                if trip_num > 100:  # Reasonable upper bound for trip numbers
                    warnings.append(
                        f"Suspicious trip number: {trip_num} (may be hallucinated)"
                    )
            except ValueError:
                pass
        ...
        if inbound_pu and len(filled_drops) > 0:
            ...
            for drop_field in filled_drops:
                if data[drop_field] == inbound_pu:
                    warnings.append(
                        f"Possible field duplication: {drop_field} and inbound_pu have same value"
                    )

        # Check for gaps in drop sequence (might indicate swapping)
        if not data.get("second_drop") and data.get("third_drop"):
            warnings.append(
                "Suspicious: 3rd drop filled but 2nd drop empty (possible field swap)"
            )

        if not data.get("third_drop") and data.get("forth_drop"):
            warnings.append(
                "Suspicious: 4th drop filled but 3rd drop empty (possible field swap)"
            )
    ...
    def _check_duplicate_locations(self, data: Dict) -> List[str]:
        ...
        if duplicate_locations:
            for dup_loc in duplicate_locations:
                warnings.append(
                    f"Duplicate location found: '{dup_loc}' appears in multiple fields"
                )
        ...
        try:
            ...
            def extract_city_state(value_str: str):
                ...
                if state_part:
                    ...
                    if len(state_norm) != 2:
                        state_norm = self.state_abbreviations.get(
                            state_part.lower(), state_part.upper()[:2]
                        )
                else:
                    state_norm = ""
            ...
            for city_key, states in city_to_states.items():
                if len(states) > 1:
                    ...
                    warnings.append(
                        f"City used with different states: '{city_display}' in {sorted(states)} ({examples})"
                    )
        except Exception:
        ...
        if data.get("total_miles"):
            ...
            try:
                ...
                if miles_num < config.MIN_TOTAL_MILES:
                    warnings.append(
                        f"Suspicious total miles (too low): {total_miles} - verify against image"
                    )
                elif miles_num > config.MAX_TOTAL_MILES:
                    warnings.append(
                        f"Suspicious total miles (too high): {total_miles} - verify against image"
                    )
            except ValueError:
