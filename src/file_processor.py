...
class FileProcessor:
    ...
    def process_folder(self, input_folder: str, use_here_api: bool = True) -> List[Dict]:
        """
            ...
            for i, image_path in enumerate(image_files, 1):
                try:
                    ...
                    if self.main_processor:
                        result = self.main_processor.process_image_with_distances(image_path, use_here_api)
                    else:
    ...
    def filter_successful_results(self, results: List[Dict]) -> List[Dict]:
        ...
        self.logger.info(f"Filtered {len(successful_results)} successful results from {len(results)} total")
    ...
    def get_processing_summary(self, results: List[Dict]) -> Dict:
        ...
        summary['common_errors'] = dict(sorted(error_counts.items(), key=lambda x: x[1], reverse=True))
        
        # Calculate success rates
        if summary['total_images'] > 0:
            summary['processing_success_rate'] = summary['successful_processing'] / summary['total_images']
            
            if summary['successful_processing'] > 0:
                summary['geocoding_success_rate'] = summary['geocoding_success'] / summary['successful_processing']
                summary['distance_calculation_rate'] = summary['distance_calculations'] / summary['successful_processing']
                summary['reference_validation_rate'] = summary['reference_validations'] / summary['successful_processing']
        
        self.logger.info(f"Processing Summary:")
        self.logger.info(f"  Total images: {summary['total_images']}")
        self.logger.info(f"  Successful processing: {summary['successful_processing']} ({summary.get('processing_success_rate', 0):.1%})")
        self.logger.info(f"  Geocoding success: {summary['geocoding_success']} ({summary.get('geocoding_success_rate', 0):.1%})")
        self.logger.info(f"  Distance calculations: {summary['distance_calculations']} ({summary.get('distance_calculation_rate', 0):.1%})")
        self.logger.info(f"  Reference validations: {summary['reference_validations']} ({summary.get('reference_validation_rate', 0):.1%})")
    ...
    def create_error_report(self, results: List[Dict], output_path: str) -> bool:
        ...
        """
        try:
            failed_results = [r for r in results if not r.get('processing_success', False)]
        ...
        for result in failed_results:
            ...
            if len(error_summary[error_type]['examples']) < 3:
                error_summary[error_type]['examples'].append({
                    'image': result.get('source_image', 'Unknown'),
                    'error': error
                })
        
        # Sort by frequency
        sorted_summary = dict(sorted(error_summary.items(), key=lambda x: x[1]['count'], reverse=True))
