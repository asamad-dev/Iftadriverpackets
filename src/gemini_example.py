    ...
    while True:
        try:
            ...
            if 1 <= choice_num <= len(image_files):
                ...
                print(f"❌ Please enter a number between 1 and {len(image_files)} (or 0 to cancel)")
...
def process_single_image_complete(image_path: str):
    ...
    try:
        ...
        print(f"❌ Failed to initialize processor: {e} at line {line_no} in {os.path.basename(fname)}")
        traceback.print_exc()
        return None
    ...
    if result.get('processing_success'):
        ...
        output_dir = os.path.join(os.path.dirname(os.path.dirname(image_path)), 'output')
        ...
        populated_fields = sum(1 for k, v in result.items() if v and str(v).strip() and k not in ['source_image', 'processing_success', 'error'])
        print(f"\n📊 Extraction Summary:")
        ...
        if 'reference_validation' in result and result['reference_validation'].get('validation_success'):
            ...
            if accuracy.get('critical_discrepancies', 0) > 0:
                print(f"   🔴 Critical discrepancies: {accuracy.get('critical_discrepancies', 0)}")
            if accuracy.get('high_discrepancies', 0) > 0:
                print(f"   🟠 High severity discrepancies: {accuracy.get('high_discrepancies', 0)}")
            if accuracy.get('medium_discrepancies', 0) > 0:
                print(f"   🟡 Medium severity discrepancies: {accuracy.get('medium_discrepancies', 0)}")
            if accuracy.get('low_discrepancies', 0) > 0:
                print(f"   🔵 Low severity discrepancies: {accuracy.get('low_discrepancies', 0)}")
...
def process_batch_complete():
    ...
    api_name = "HERE (with distance calculation)" if here_available else "Nominatim (coordinates only)"
    print(f"🌍 Using {api_name} for all images")
    
    # Process all images
    results = processor.process_multiple_images(input_folder, use_here_api=here_available)
    ...
    json_output_path = os.path.join(output_folder, f'batch_results_complete_{timestamp}.json')
    with open(json_output_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    
    # Create state mileage CSV (as per plan.md)
    csv_output_path = os.path.join(output_folder, f'batch_state_mileage_{timestamp}.csv')
    ...
    if successful_extractions:
        total_geocoding_success = sum(r.get('geocoding_summary', {}).get('successful_geocoding', 0) for r in successful_extractions)
        total_locations = sum(r.get('geocoding_summary', {}).get('total_locations', 0) for r in successful_extractions)
        
        if total_locations > 0:
            print(f"   📍 Locations geocoded: {total_geocoding_success}/{total_locations} ({total_geocoding_success/total_locations:.1%})")
    
    # Print distance calculation statistics
    distance_success = sum(1 for r in results if 'distance_calculations' in r and r['distance_calculations'].get('calculation_success'))
    if distance_success > 0:
        print(f"   📏 Distance calculations successful: {distance_success}/{len(results)}")
    
    # Print validation statistics
    validation_results = [r for r in results if r.get('reference_validation', {}).get('validation_success')]
    if validation_results:
        total_field_accuracy = sum(r.get('reference_validation', {}).get('accuracy_metrics', {}).get('field_accuracy', 0) for r in validation_results)
        avg_field_accuracy = total_field_accuracy / len(validation_results)
        
        total_discrepancies = sum(r.get('reference_validation', {}).get('accuracy_metrics', {}).get('total_discrepancies', 0) for r in validation_results)
        total_critical = sum(r.get('reference_validation', {}).get('accuracy_metrics', {}).get('critical_discrepancies', 0) for r in validation_results)
        total_high = sum(r.get('reference_validation', {}).get('accuracy_metrics', {}).get('high_discrepancies', 0) for r in validation_results)
        
        print(f"\n📊 Validation Summary:")
        print(f"   Images with reference data: {len(validation_results)}/{len(results)}")
    ...
    warnings_count = sum(1 for r in results if 'validation_warnings' in r and r['validation_warnings'])
    if warnings_count > 0:
        print(f"   ⚠️  Images with warnings: {warnings_count}")

    print(f"\n💾 Output files saved:")
    print(f"   📄 State Mileage CSV: {os.path.basename(csv_output_path)} (as per plan.md)")
    print(f"   📄 Detailed CSV: {os.path.basename(detailed_csv_path)} (all extraction data)")
    print(f"   📄 JSON Details: {os.path.basename(json_output_path)}")
    print(f"   📂 Location: {output_folder}")
...
def create_detailed_csv(results, csv_output_path):
    ...
    field_order = [
        'source_image', 'processing_success', 
        'drivers_name', 'unit', 'trailer',
        'date_trip_started', 'date_trip_ended', 'trip', 'trip_started_from',
        'first_drop', 'second_drop', 'third_drop', 'forth_drop', 'inbound_pu',
        'drop_off', 'total_miles', 'geocoded_locations', 'total_distance_miles'
    ]
    ...
    with open(csv_output_path, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for result in results:
            ...
            # Add geocoding info
            if 'geocoding_summary' in result:
                row_data['geocoded_locations'] = result['geocoding_summary'].get('successful_geocoding', 0)
            
            # Add distance calculations if available
            if 'distance_calculations' in result and 'total_distance_miles' in result['distance_calculations']:
                row_data['total_distance_miles'] = result['distance_calculations'].get('total_distance_miles', 0)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--help":
            ...
            print("\n🔧 Requirements:")
            print("  • GEMINI_API_KEY: Required (get from https://makersuite.google.com/)")
            print("  • HERE_API_KEY: Optional for distance calculation (get from https://developer.here.com/)")
        else:
    ...
    try:
        ...
        for i, entry in enumerate(reference_entries[:5]):
            ...
            if result.get('processing_success'):
                ...
                if validation.get('validation_success') and validation.get('reference_found'):
                    accuracy = validation.get('accuracy_metrics', {})
                    
                    test_results.append({
                        'image_name': image_name,
                        'field_accuracy': accuracy.get('field_accuracy', 0),
                        'total_discrepancies': accuracy.get('total_discrepancies', 0),
                        'critical_discrepancies': accuracy.get('critical_discrepancies', 0),
                    ...
                    if accuracy.get('critical_discrepancies', 0) > 0:
                        print(f"   🔴 Critical issues: {accuracy.get('critical_discrepancies', 0)}")
                else:
        ...
        if test_results:
            ...
            avg_accuracy = sum(r['field_accuracy'] for r in test_results) / len(test_results)
            ...
            problematic_images = [r for r in test_results if r['critical_discrepancies'] > 0 or r['high_discrepancies'] > 2]
            if problematic_images:
                print(f"\n⚠️  Images requiring attention:")
                for img in problematic_images:
                    print(f"   - {img['image_name']}: {img['field_accuracy']:.1%} accuracy, {img['total_discrepancies']} discrepancies")
