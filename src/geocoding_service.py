...
class GeocodingService:
    ...
    def __init__(self, here_api_key: Optional[str] = None):
        ...
        if self.here_api_key:
            ...
            self.logger.warning("HERE API key not available - will use Nominatim as fallback")
    
    def geocode_location(self, location: str, use_here_api: bool = True) -> Optional[Tuple[float, float]]:
        """
    ...
    def _geocode_here(self, location: str) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            if data.get('items') and len(data['items']) > 0:
                ...
                self.logger.debug(f"HERE geocoding failed for '{location}' - no results")
                return None
    ...
    def _geocode_nominatim(self, location: str) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            if data and len(data) > 0:
                ...
                # Cache negative result
                self.geocoding_cache[location] = None
                self.logger.debug(f"Nominatim geocoding failed for '{location}' - no results")
                return None
    ...
    def get_coordinates_for_stops(self, extracted_data: Dict, use_here_api: bool = True) -> Dict:
        ...
        for field in location_fields:
            ...
            if location and location.strip():
                ...
                if coords:
                    ...
                    self.logger.debug(f"Found coordinates: {coords[0]:.6f}, {coords[1]:.6f}")
                else:
                    coordinates[field] = {
                        'location': location,
                        'latitude': None,
        ...
        self.logger.info(f"Geocoding completed: {successful_coords}/{total_locations} successful "
    ...
    def get_cache_stats(self) -> Dict:
        ...
        successful_entries = sum(1 for v in self.geocoding_cache.values() if v is not None)
