...
class DriverPacketProcessor:
    ...
    def __init__(self, 
        ...
        try:
            ...
            # State analyzer
            self.state_analyzer = StateAnalyzer(geocoding_service=self.geocoding_service)
            ...
            # Reference validator
            self.reference_validator = ReferenceValidator(reference_csv_path=reference_csv_path)
    ...
    def process_single_image(self, image_path: str, use_here_api: bool = True) -> Dict:
        ...
        """
        try:
            self.logger.info(f"🚛 Starting complete processing of: {os.path.basename(image_path)}")
            ...
            corrected_data, corrections = self.data_validator.validate_and_correct_data(extraction_result)
            validation_warnings = self.data_validator.validate_extracted_data(corrected_data)
            
            # Stage 3: Get coordinates for locations
            self.logger.info("🌍 Stage 3: Getting coordinates for locations...")
            coordinates_data = self.geocoding_service.get_coordinates_for_stops(corrected_data, use_here_api)
            
            # Stage 4: Calculate route distances
            self.logger.info("📏 Stage 4: Calculating route distances...")
            distance_data = self.route_analyzer.calculate_trip_distances(coordinates_data)
            
            # Stage 5: Analyze state mileage distribution
            self.logger.info("🗺️ Stage 5: Analyzing state mileage distribution...")
            polylines = distance_data.get('trip_polylines', []) if distance_data.get('calculation_success') else []
            enhanced_distance_data = self.state_analyzer.add_state_mileage_to_trip_data(distance_data, polylines)
            
            # Stage 6: Validate against reference data (if available)
            self.logger.info("🔍 Stage 6: Validating against reference data...")
            reference_validation = self.reference_validator.validate_against_reference(corrected_data)
            
            # Stage 7: Compare extracted vs calculated miles
            if corrected_data.get('total_miles') and enhanced_distance_data.get('total_distance_miles'):
            ...
            self.logger.info(f"✅ Complete processing finished for {os.path.basename(image_path)}")
    ...
    def process_multiple_images(self, input_folder: str, use_here_api: bool = True) -> List[Dict]:
        """
    ...
    def generate_accuracy_report(self, results: List[Dict]) -> Dict:
        ...
        if not validation_results:
            self.logger.warning("No reference validation results found for accuracy report")
            return {}
    ...
    def _generate_processing_summary(self, result: Dict) -> Dict:
        ...
        if coords and coords.get('geocoding_summary', {}).get('successful_geocoding', 0) > 0:
    ...
    def get_coordinates_for_stops(self, extracted_data: Dict, use_here_api: bool = True) -> Dict:
        ...
        """
        return self.geocoding_service.get_coordinates_for_stops(extracted_data, use_here_api)
    ...
    def get_cache_stats(self) -> Dict:
        ...
        stats = {
            'geocoding_cache': self.geocoding_service.get_cache_stats()
        }
...
def process_driver_packet(image_path: str, 
                         gemini_api_key: Optional[str] = None,
                         here_api_key: Optional[str] = None,
                         use_here_api: bool = True) -> Dict:
    """
...
def process_driver_packet_folder(input_folder: str,
                                output_folder: str,
                                gemini_api_key: Optional[str] = None,
                                here_api_key: Optional[str] = None,
                                use_here_api: bool = True) -> List[Dict]:
    """
