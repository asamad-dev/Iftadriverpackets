...
class ReferenceValidator:
    ...
    def validate_against_reference(self, extracted_data: Dict) -> Dict:
        ...
        try:
            ...
            if not os.path.exists(self.reference_csv_path):
                self.logger.info(
                    f"No reference test file available (skipping validation): {os.path.basename(self.reference_csv_path)}"
                )
            ...
            if not reference_entry:
                validation_result["validation_warnings"].append(
                    f"No reference data found for image: {source_image}"
                )
                return validation_result
            ...
            discrepancies = self._compare_extracted_vs_reference(
                extracted_data, reference_entry
            )
            validation_result["discrepancies"] = discrepancies

            # Calculate accuracy metrics
            accuracy_metrics = self._calculate_accuracy_metrics(
                extracted_data, reference_entry, discrepancies
            )
            ...
            self.logger.info(
                f"  Field accuracy: {accuracy_metrics.get('field_accuracy', 0):.1%}"
            )
            ...
            if discrepancies:
                ...
                for discrepancy in discrepancies[:5]:  # Show first 5
                    self.logger.warning(
                        f"  - {discrepancy['field']}: {discrepancy['extracted']} ≠ {discrepancy['reference']}"
                    )
    ...
    def _load_reference_data(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.reference_data)} reference entries from CSV"
            )
            return True
    ...
    def _find_reference_entry(self, source_image: str) -> Optional[Dict]:
        ...
        image_name_base = (
            source_image.replace(".jpg", "").replace(".jpeg", "").replace(".png", "")
        )
        for entry in self.reference_data:
    ...
    def _compare_extracted_vs_reference(
        self, extracted_data: Dict, reference_entry: Dict
    ) -> List[Dict]:
        ...
        for extracted_field, reference_field in self.field_mapping.items():
            extracted_value = self._normalize_field_value(
                extracted_data.get(extracted_field, "")
            )
            reference_value = self._normalize_field_value(
                reference_entry.get(reference_field, "")
            )

            # Special handling for drop_off arrays
            if extracted_field == "drop_off" and isinstance(
                extracted_data.get("drop_off"), list
            ):
                # Check if reference has multiple drop offs
                drop_off_fields = ["Drop Off", "Drop Off 2", "Drop Off 3"]
                reference_drop_offs = []
                for drop_field in drop_off_fields:
                    drop_value = self._normalize_field_value(
                        reference_entry.get(drop_field, "")
                    )
                    if drop_value:
                ...
                # Compare arrays
                extracted_drop_offs = [
                    self._normalize_field_value(v)
                    for v in extracted_data.get("drop_off", [])
                ]
            ...
            if extracted_value != reference_value:
                severity = self._determine_discrepancy_severity(
                    extracted_field, extracted_value, reference_value
                )
                discrepancies.append(
                    {
                        "field": extracted_field,
    ...
    def _determine_discrepancy_severity(
        self, field: str, extracted: str, reference: str
    ) -> str:
    ...
    def _is_minor_formatting_difference(self, extracted: str, reference: str) -> bool:
        ...
        extracted_clean = (
            extracted.lower().replace(".", "").replace("-", "/").replace(" ", "")
        )
        reference_clean = (
            reference.lower().replace(".", "").replace("-", "/").replace(" ", "")
        )

        return extracted_clean == reference_clean

    def _calculate_accuracy_metrics(
        self, extracted_data: Dict, reference_entry: Dict, discrepancies: List[Dict]
    ) -> Dict:
        """Calculate accuracy metrics"""
        ...
        medium_discrepancies = sum(
            1 for d in discrepancies if d["severity"] == "medium"
        )
    ...
    def _generate_validation_warnings(self, discrepancies: List[Dict]) -> List[str]:
        ...
        for discrepancy in discrepancies:
            ...
            if severity == "critical":
                warnings.append(
                    f"🔴 CRITICAL: {field} mismatch - extracted: '{extracted}' ≠ reference: '{reference}'"
                )
            elif severity == "high":
                warnings.append(
                    f"🟠 HIGH: {field} mismatch - extracted: '{extracted}' ≠ reference: '{reference}'"
                )
            elif severity == "medium":
                warnings.append(
                    f"🟡 MEDIUM: {field} mismatch - extracted: '{extracted}' ≠ reference: '{reference}'"
                )
            else:
                warnings.append(
                    f"🔵 LOW: {field} formatting difference - extracted: '{extracted}' ≠ reference: '{reference}'"
                )
    ...
    def generate_accuracy_report(self, validation_results: List[Dict]) -> Dict:
            ...
            if result.get("validation_success"):
                ...
                if result.get("reference_found"):
                    ...
                    if metrics:
                        ...
                        report["severity_breakdown"]["critical"] += metrics.get(
                            "critical_discrepancies", 0
                        )
                        report["severity_breakdown"]["high"] += metrics.get(
                            "high_discrepancies", 0
                        )
                        report["severity_breakdown"]["medium"] += metrics.get(
                            "medium_discrepancies", 0
                        )
                        report["severity_breakdown"]["low"] += metrics.get(
                            "low_discrepancies", 0
                        )
        ...
        if total_fields_checked > 0:
            report["overall_field_accuracy"] = (
                total_fields_correct / total_fields_checked
            )
        ...
        self.logger.info(
            f"  Successfully validated: {report['successfully_validated']}"
        )
        self.logger.info(f"  Reference matches: {report['reference_matches_found']}")
        self.logger.info(
            f"  Overall field accuracy: {report['overall_field_accuracy']:.1%}"
        )
        ...
        if report["common_issues"]:
            ...
            for issue in report["common_issues"][:5]:  # Show top 5 issues
                self.logger.warning(
                    f"    {issue['field']}: {issue['accuracy']:.1%} ({issue['correct']}/{issue['total_checked']})"
                )
