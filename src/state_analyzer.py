...
class StateAnalyzer:
    ...
    def __init__(self, geocoding_service: Optional[GeocodingService] = None):
        ...
        if GIS_AVAILABLE:
            self.logger.info("GIS dependencies available - enhanced route analysis enabled")
        else:
            self.logger.warning("GIS dependencies not available - using fallback state analysis")
    
    def load_state_boundaries(self):
        """Load and prepare state boundary data from shapefiles"""
        if not GIS_AVAILABLE:
            self.logger.warning("GIS dependencies not available - cannot load state boundaries")
            return None
        ...
        if self._state_boundaries is None:
            ...
            try:
                ...
                self.logger.info(f"Loaded {len(self._state_boundaries)} state boundaries")
    ...
    def calculate_state_miles_from_polyline(self, polyline_str, total_distance_miles: float) -> Dict[str, float]:
        ...
        """
        if not GIS_AVAILABLE:
            self.logger.warning("GIS dependencies not available - cannot perform polyline analysis")
            return {}
        ...
        try:
            ...
            if states_gdf is None:
                self.logger.warning("State boundaries not available - cannot perform intersection")
                return {}
            ...
            self.logger.info(f"Created route geometry from {len(combined_line_geoms)} segment(s), {total_points} points total")
            ...
            state_miles = {state: miles for state, miles in state_miles.items() if miles >= config.MIN_STATE_MILES_THRESHOLD}
    ...
    def analyze_route_states_enhanced(self, polyline: Optional[str], origin_coords: Tuple[float, float], 
        ...
        """
        try:
            self.logger.info(f"Enhanced route analysis: {total_distance_miles:.1f} mile route")
            
            # Generate strategic sample points along the route path
            sample_points = self._generate_route_sample_points(origin_coords, destination_coords, total_distance_miles)
            
            self.logger.debug(f"Analyzing {len(sample_points)} strategic sample points...")
            ...
            for i, point in enumerate(sample_points):
                try:
                    ...
                    if state:
                        ...
                        self.logger.debug(f"Point {i+1}/{len(sample_points)}: {state} at {point[0]:.4f},{point[1]:.4f}")
            ...
            if not states_encountered:
                ...
                return self._fallback_endpoint_state_analysis(origin_coords, destination_coords, total_distance_miles)
            
            # Enhanced state mileage estimation
            return self._calculate_enhanced_state_mileage(states_encountered, total_distance_miles)
            
        except Exception as e:
            self.logger.error(f"Enhanced analysis failed: {e}")
            return self._fallback_endpoint_state_analysis(origin_coords, destination_coords, total_distance_miles)
    
    def _generate_route_sample_points(self, origin: Tuple[float, float], destination: Tuple[float, float], 
        ...
        return enhanced_points[:config.ROUTE_SAMPLE_POINTS_MAX]  # Cap at configured limit to avoid API rate limits
    ...
    def _calculate_enhanced_state_mileage(self, states_encountered: List[Dict], total_distance_miles: float) -> Dict:
        """
        ...
        for segment in state_segments:
            ...
            if distance < 5.0:  # Less than 5 miles might be GPS noise
                self.logger.debug(f"Very short segment in {state} ({distance:.1f} mi) - might be border noise")
        ...
        self.logger.info(f"Enhanced analysis: {len(states_list)} states, {coverage_percentage:.1f}% route coverage")
        for state_data in states_list:
            self.logger.debug(f"  {state_data['state']}: {state_data['miles']} miles ({state_data['percentage']}%)")
    ...
    def _fallback_endpoint_state_analysis(self, origin_coords: Tuple[float, float], 
        ...
        origin_state = self._reverse_geocode_to_state(origin_coords) if self.geocoding_service else None
        dest_state = self._reverse_geocode_to_state(destination_coords) if self.geocoding_service else None
    ...
    def add_state_mileage_to_trip_data(self, trip_distance_data: Dict, polylines: Optional[List[str]] = None) -> Dict:
        """
        ...
        if polylines and GIS_AVAILABLE:
            state_miles = self.calculate_state_miles_from_polyline(polylines, total_distance)
        elif origin_coords and destination_coords:
            # Use enhanced sampling analysis
            state_analysis = self.analyze_route_states_enhanced(None, origin_coords, destination_coords, total_distance)
            state_miles = {state['state']: state['miles'] for state in state_analysis.get('states', [])}
        else:
            state_miles = {}
